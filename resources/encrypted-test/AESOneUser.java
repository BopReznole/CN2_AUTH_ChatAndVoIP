//THIS SHOULD REPLACE THE AESci.java
//IT IS MEANT TO BE USED FOR CHATTING IN A SINGLE DEVICE
...
//    this.IV = decode(ivstr); 	// Unecessary, used to directly assign the byte array to the real IV upon generating a new
  ...
  public void setIV(String IVnew, int i) {
	  	if (!IVnew.isEmpty()) { // Checks if the string is empty
	  		if(i<1) {
	  			this.IVsend = decode(IVnew);
	  		}
	  		else {
	  			this.IVrecieve = decode(IVnew);
	  		}
...
//        byte[] messageInBytes = message.getBytes(StandardCharsets.UTF_8); // This didn't work somehow
        ...
        GCMParameterSpec spec = new GCMParameterSpec(T_LEN, IVsend); // Creates a GCMParameterSpec object with the given tag length (T_LEN) and IV
    ...
    public String decrypt(String encryptedMessage) throws Exception {
        ...
        GCMParameterSpec spec = new GCMParameterSpec(T_LEN, IVrecieve); // Creates a GCMParameterSpec object with the same tag length and IV used for encryption
...
//        return new String(decryptedBytes, StandardCharsets.UTF_8); // This didn't work somehow
    ...
    public void exportKeys() {
    	...
    	System.err.println("IVsend (base64 encoded): " + encode(IVsend));
    	System.err.println("IVrecieve (base64 encoded): " + encode(IVrecieve));
...
//    	setIV(ivstr); //sets the new IV which was previously generated //ONLY WORKS FOR SEPARATE DEVICES
    	setIV(ivstr,0); //sets the new IV which was previously generated //ONLY WORKS FOR SEPARATE DEVICES
...
//	 		setIV(IVnew);  //Sets the new IV from IVnew //ONLY WORKS FOR SEPARATE DEVICES
	 		setIV(IVnew,1);  //Sets the new IV from IVnew //ONLY WORKS FOR SEPARATE DEVICES
...
//	 		setIV(IVnew);  //Sets the new IV //ONLY WORKS FOR SEPARATE DEVICES
	 		setIV(IVnew,1);  //Sets the new IV //ONLY WORKS FOR SEPARATE DEVICES
